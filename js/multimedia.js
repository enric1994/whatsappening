$(document).ready(function () {
  ...
  $.get('./stats.csv', function (csvString) {
    ...
    for (i = 0; i < data_raw.length; i++) {
      ...
      processed_data['video'].push(data_raw[i].video);
      processed_data['image'].push(data_raw[i].image);
      processed_data['audio'].push(data_raw[i].audio);
    }
      dataset.push({
        label: 'Videos',
    ...
    new Chart(document.getElementById("multimedia"), {
      ...
      options: {
        ...
        title: {
          display: true,
          text: 'Videos, images and audios per character'
        },
        scales: {
          xAxes: [{
            stacked: true,
